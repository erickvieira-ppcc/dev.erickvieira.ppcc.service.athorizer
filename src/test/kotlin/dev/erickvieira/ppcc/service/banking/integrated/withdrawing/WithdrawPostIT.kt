package dev.erickvieira.ppcc.service.banking.integrated.withdrawing
...
@DirtiesContext
class WithdrawPostIT : BankingServiceIntegratedTests() {
    ...
    fun setUp() {
        ...
        RestAssured.basePath = "/api/v1/banking/withdraw"
    }
    ...
    @ExpectedDataSet(
        "withdrawing/scenario-approved.yml",
        compareOperation = CompareOperation.CONTAINS
    )
    fun `withdraw - must debit the given wallet`() {
    ...
    fun `withdraw - must debit the default wallet of the given user`() {
        val payload = generateTransactionDTO(
    ...
    fun `withdraw - must decline with error declined_by_wallet_not_found`() {
        val payload = generateTransactionDTO(
    ...
    fun `withdraw - must decline with error declined_by_invalid_value`() {
        val payload = generateTransactionDTO(
            userId = UUID.fromString("9c05df92-4044-54ee-964c-ab5aee0b4f57"),
            value = 0.00
        )
    ...
    fun `withdraw - must decline with error declined_by_null_payload`() {
        val payload = null
        val responseBody = makeRequest(
            payload = payload,
            statusCode = HttpStatus.BAD_REQUEST
    ...
    fun `withdraw - must decline with error declined_by_insufficient_funds`() {
        val payload = generateTransactionDTO(
            userId = UUID.fromString("9c05df92-4044-54ee-964c-ab5aee0b4f57"),
            value = 1000.00
        )
    ...
    fun `withdraw - must decline with error declined_by_min_balance`() {
        val payload = generateTransactionDTO(
            userId = UUID.fromString("9c05df92-4044-54ee-964c-ab5aee0b4f57"),
            value = 60.00
        )
    ...
    @Test
    @DataSet("withdrawing/scenario-accept-withdrawing-false.yml")
    @ExpectedDataSet(
        "withdrawing/scenario-declined-forbidden-operation.yml",
        compareOperation = CompareOperation.CONTAINS
    )
    fun `withdraw - must decline with error declined_by_forbidden_operation`() {
        val payload = generateTransactionDTO(
            userId = UUID.fromString("9c05df92-4044-54ee-964c-ab5aee0b4f57"),
            value = 10.00
        )
    ...
    fun `withdraw - must decline with error declined_by_forbidden_operation - alt`() {
        val payload = generateTransactionDTO(
            userId = UUID.fromString("9c05df92-4044-54ee-964c-ab5aee0b4f57"),
            value = 10.00
        )
