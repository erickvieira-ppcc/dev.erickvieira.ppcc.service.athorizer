package dev.erickvieira.ppcc.service.banking
...
@EnableEurekaClient
@ComponentScan(basePackages = ["dev.erickvieira.ppcc", "dev.erickvieira.ppcc.service.banking"])
open class BankingServiceApplication

val logger: Logger = LoggerFactory.getLogger(BankingServiceApplication::class.java)

fun main(
    args: Array<String>
) = SpringApplication(BankingServiceApplication::class.java).run(*args).environment.let { env ->
    ...
    val contextPath = env.getProperty("server.servlet.context-path").let {
        if (it.isNullOrBlank()) "api/v1/banking" else it
    }
